...
export const SessionReport = () => {
  ...
  const handleReport= async () => {
   ...
   const generatePDF = (data, isAll) => {
     ...
     // Set the file name based on whether "All" or a specific tutor is selected
     const fileName = isAll ? "All_Tutor_Reports.pdf" : `${data.data[0].booking_details.first_name +''+data.data[0].booking_details.last_name}_Report.pdf`;
     doc.save(fileName);
